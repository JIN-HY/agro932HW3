...
#df <- read.table("largedata/RiceDiversity_44K_Genotypes_PLINK/binary_sativas413.ld", header = T)
BINSIZE = 100
...
#df2 <- df %>% group_by(bin) %>% summarise(meanr2 = mean(R2))
...
library(ggplot2)
ggplot(pheno, aes(x=Flowering.time.at.Arkansas)) + 
  geom_histogram(aes(y=..density..), bins=50, fill="#999999")+
  geom_density(alpha=.2, fill="#FF6666") +
  labs(title="Phenotype histogram plot",x="Flowering.time.at.Arkansas", y = "Density")+
  theme_classic()
...
```{bash, eval=FALSE}
gemma -bfile binary_sativas413 -k output/binary_sativas413.cXX.txt -c pc3.txt -p pheno.txt -lmm 4 -n 3 -o Flowering.time.at.Arkansas -miss 0.9 -r2 1 -hwe 0 -maf 0.05

cp output/Flowering.time.at.Arkansas.assoc.txt ../../../cache
```
...
library(qqman)
library("data.table")
res <- fread("cache/Flowering.time.at.Arkansas.assoc.txt")
pdf("graphs/manhattan.pdf")
manhattan(x = res, chr = "chr", bp = "ps", p = "p_wald", snp = "rs", col = c("blue4", "orange3"), logp = TRUE, genomewideline = -log10(0.05/36901))
dev.off()
```
